        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-emerald-950/40 hover:bg-emerald-900/50 border border-emerald-500/30 text-emerald-400 text-xs font-medium transition-all group"
              title="Click to view data sync timings and update intervals"
            >
            ...
            <div className="relative">
              ...
              {langDropdownOpen && (
                <div className="absolute right-0 mt-2 w-44 rounded-xl bg-slate-800 border border-slate-700 shadow-2xl py-1 z-50">
                  {LANGUAGES.map((lang) => (
                    <button
                      key={lang.code}
                      onClick={() => {
      ...
      <div className="flex md:hidden overflow-x-auto px-4 py-2 border-t border-slate-800/80 gap-2 bg-slate-900/90 text-xs">
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg whitespace-nowrap ${
            activeTab === 'map' ? 'bg-brand-600 text-white' : 'text-slate-300 bg-slate-800'
          }`}
        >
        ...
        <button
          onClick={() => setActiveTab('calculator')}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg whitespace-nowrap ${
            activeTab === 'calculator' ? 'bg-brand-600 text-white' : 'text-slate-300 bg-slate-800'
          }`}
        >
        ...
        <button
          onClick={() => setActiveTab('openDataHub')}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg whitespace-nowrap ${
            activeTab === 'openDataHub' ? 'bg-brand-600 text-white' : 'text-slate-300 bg-slate-800'
          }`}
        >
